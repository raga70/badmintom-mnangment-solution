...
public class RoundRobinTests
{
    ...
    private readonly TournamentInPlay Tournament = new TournamentRR(sampleTournament);
    ...
    private bool validate(List<Fight> fights, List<User> users)
    {
        foreach (var user in users)
        {
            ...
            foreach (var fight in fights) //check if all users are in the fights and fight eachother 
                ...
                {
                    if (!allUsersExceptThisOne.Contains(fight.Player2.Player)) return false;
                }
                else if (fight.Player2.Player == user)
                {
                    if (!allUsersExceptThisOne.Contains(fight.Player1.Player)) return false;
                }
