...
public class SingleEliminationTests
{
     ...
     static Tournament sampleTournament = new Tournament()
    {
     ...
     private TournamentInPlay Tournament = new TournamentSE(sampleTournament);
     ...
     [TestMethod]
        public void testPlayersOnThePowerOf2_secondRound()
        {
            ...
            MethodInfo method = Tournament.GetType().GetMethod("CreteSchedule", BindingFlags.NonPublic | BindingFlags.Instance);
     ...
     bool validate(List<Fight> fights, List<User> users) 
     {
         foreach (var user in users)
         {
             ...
             if(fights.Count(f => f.Player1.Player == user || f.Player2.Player == user) > 1)
             {
                 return false;
             }
