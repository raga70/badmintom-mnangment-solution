...
public abstract class TournamentInPlay : Tournament
{
    ...
    protected List<GamePlayer> _players { get; } =
        new(); //used by the initializer to prepare the players for CreteSchedule() 
    ...
    public void Initializer()
    {
        for (var i = 0; i < Players.Count; i++)
            _players.Add(new GamePlayer { id = i, Player = Players[i], Score = 0, isB = false });

        if (Players.Count % 2 == 1)
            _players.Add(new GamePlayer { id = Players.Count, Player = null, Score = 0, isB = true });
    }
    ...
    public void UpdateGame()
    {
        ...
        if (isValid && TournamentSystem == TournamentSystems.SingleElimination) CreteSchedule(_rounds);
    }
}